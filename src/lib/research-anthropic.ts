  ...
  try {
    for (let turn = 0; turn < MAX_TURNS; turn++) {
      ...
      stream.on('streamEvent', (event) => {
        ...
        if (block.type === 'thinking') emit('live', 'Thinking …', 'thinking');
      });
      stream.on('inputJson', (delta) => {
        if (writing) wrote(delta.length);
      });
      stream.on('contentBlock', (block) => {
        if (block.type === 'server_tool_use') {
          ...
          emit('note', `web_search: ${block.content.error_code}`);
      ...
      if (final.stop_reason === 'refusal') throw message('errRefusal', { name: 'Claude' });
      const submitted = final.content.find((b) => b.type === 'tool_use' && b.name === 'submit_dataset');
      if (submitted && submitted.type === 'tool_use') {
        if (final.stop_reason === 'max_tokens') throw message('errTruncated');
    ...
    throw message('errNoDataset', { name: 'Claude' });
  } catch (err) {
    throw explainApiError(err, Anthropic, 'Anthropic', model);
  }
}
